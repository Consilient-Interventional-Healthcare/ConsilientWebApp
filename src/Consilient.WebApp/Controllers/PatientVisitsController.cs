...
{
    [Authorize]
    public class PatientVisitsController(IPatientVisitsApi patientVisitsApi) : Controller
    {
        // GET: PatientVisits
        public async Task<IActionResult> Index(DateOnly? selectedDate)
        {
            selectedDate ??= DateOnly.FromDateTime(DateTime.Now.AddDays(-1));
            var patientVisits = (await patientVisitsApi.GetByDateAsync(selectedDate.Value).ConfigureAwait(false)).Unwrap()!;
