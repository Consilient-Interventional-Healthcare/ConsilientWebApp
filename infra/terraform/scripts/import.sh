...
if [ ! -f "terraform.tfstate" ] && [ ! -f ".terraform/terraform.tfstate" ]; then
  echo "ℹ️  No Terraform state file found - this appears to be a fresh deployment"
  echo "ℹ️  Skipping resource import step (will create all resources from scratch)"
  exit 0
fi
...
import_resource() {
  ...
  if terraform state list | grep -q "^${tf_address}\$"; then
    echo "  ✅ Already in state: ${resource_name}"
    return 0
  fi
...
if [ -n "$PRINCIPAL_ID" ]; then
  ...
  if [ -n "$ROLE_ASSIGNMENT_ID" ]; then
    ...
    echo "  ℹ️  No role assignment found for this managed identity on storage account"
  fi
else
  echo "  ℹ️  Managed identity not found, skipping role assignment import"
fi
...
if [ "$CREATE_CAE" = "false" ] && [ "$USE_SHARED_CAE" = "true" ] && [ -n "$EXPECTED_CAE_NAME" ]; then
  ...
  if [ -n "$CAE_EXISTS" ]; then
    ...
    echo "  ℹ️  CAE '${EXPECTED_CAE_NAME}' not found in resource group '${TF_VAR_resource_group_name}'"
    echo "  ℹ️  Terraform will use data source to reference CAE from another resource group"
  fi
elif [ "$CREATE_CAE" = "true" ]; then
  echo "  ℹ️  create_container_app_environment=true - Terraform will create a new CAE"
  echo "  ℹ️  Attempting to import in case it already exists..."
  ...
  echo "  ℹ️  CAE configuration doesn't require import (using existing CAE via data source or ID)"
fi
