...
BACKGROUNDHOST_APP_NAME=$(terraform output -raw backgroundhost_app_service_name 2>/dev/null || echo "")
...
if [[ "${ACTIONS_STEP_DEBUG}" == "true" ]]; then
  ...
  echo "  BackgroundHost App Name: ${BACKGROUNDHOST_APP_NAME}"
...
echo "backgroundhost_app_name=${BACKGROUNDHOST_APP_NAME}" >> $GITHUB_OUTPUT
